...
int Infantry::getAttackScore() {
    ...
    if (personalNumber > 7) {
        this->quantity += ceil(this->quantity * 0.2);
    }
    else if (personalNumber < 3) {
        this->quantity -= ceil(this->quantity * 0.1);
    }
...
TerrainElement::TerrainElement(): pos(new Position()) {}
...
// class Mountain
void Mountain::updateArmyScore(vector<Unit*>& units, double disThreshold, double ratio, double& score, bool increase) {
...
void HCMCampaign::run() {
    ...
    if (eventCode < 75) {
        ...
        this->liberationArmy->fight(this->arvn, false);
        this->liberationArmy->getUnitList()->removeWithQuantity(5);
        this->arvn->getUnitList()->removeWithQuantity(5);
    }
}
